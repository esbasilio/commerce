...
use App\Models\PreventativeHasClient;
...
class ClientController extends Component
{
	...
	public $name,$phone,$email, $status,$profile = 'Cliente', $image,$password, $selected_id, $fileLoaded, $role;
	...
	public function render()
	{
		...
		if(Auth::user()->profile == 'Admin' || Auth::user()->profile == 'Comercio'){
			$filters = ['users.comercio_id' => Auth::user()->comercio_id];
		}else{
			$filters = ['phc.preventist_id' => Auth::user()->id, 'users.comercio_id' => Auth::user()->comercio_id];
		}
		...
		if(strlen($this->search) > 0)
			...
			$data = User::select('*')
			->join('preventative_has_clients as phc', 'users.id', 'phc.client_id')
			->where($filters)
			->orderBy('name','asc')->paginate($this->pagination);

		return view('livewire.clients.component',[
			'data' => $data,
			'roles' => Role::whereSlug('client')->get()
		])
	...
	public function Store()
	{
        ...
        PreventativeHasClient::firstOrCreate([
            'preventist_id' => Auth::user()->id,
            'client_id'     => $user->id,
            'shop_id'       => $comercio_id
        ]);

		$this->resetUI();
		$this->emit('user-added', 'Cliente Registrado Correctamente');
